...
> **🤝 充满人味，不止于冷冰冰的 AI：** 打造你的私人新闻雷达。支持深度自定义，并鼓励大家通过 [horizon1123.top](https://horizon1123.top/) 贡献并分享高质量新闻。
...
- **🔔 Webhook 通知** — 将结果推送至飞书、Slack、Discord 或任意 Webhook 端点，支持模板渲染与截断
...
向导会询问你的兴趣（如"LLM 推理"、"嵌入式"、"web 安全"），自动推荐并生成 `data/config.json`，还可选让 AI 补充推荐小众源。若你想贡献新闻，请前往 [horizon1123.top](https://horizon1123.top/)。
...
- <details><summary>钉钉</summary>
  ...
  - 在.env的`HORIZON_WEBHOOK_URL`中输入钉钉机器人的 `Webhook地址`
  - request_body中输入
    ```json
    {
        "msgtype": "markdown",
...
- <details><summary>飞书</summary>
  ...
  - 在.env的`HORIZON_WEBHOOK_URL`中输入飞书机器人的 `Webhook地址`
  - request_body中输入
    ```json
    {
...
- [x] **Webhook 通知** — 将结果推送至飞书、Slack、Discord 或任意 Webhook 端点
...
想把有价值的资讯分享给 Horizon 社区？请直接前往 **[horizon1123.top](https://horizon1123.top)** 贡献新闻。
